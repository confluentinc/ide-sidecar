...
/**
 * Enum holding websocket message types of concern to sidecar. Other message types may exist for use
 * for IDE workspace <--> workspace communication.
 **/
...
public enum MessageType {
  ...
  /**
   * Message sent by the workspace to the sidecar when it first connects, combined with a HelloBody
   * body payload. Receipt of this message (containing a valid workspace id in the body) is what
   * will mark this as a good session and cause an increase in sidecar's workspace count.
   */
  WORKSPACE_HELLO,

  /**
   * Message sent by the sidecar to all workspaces when count of connected workspaces (that have
   * sent proper WORKSPACE_HELLO messages) has increased or decreased.
   */
  WORKSPACE_COUNT_CHANGED,

  /**
   * Message sent by sidecar to a workspace when sidecar has noticed a websocket messaging error and
   * is going to disconnect its end of the websocket.
   */
  PROTOCOL_ERROR,

  /**
   * Message sent by sidecar to workspaces when a connection has been created, changed, deleted, or
   * its status has changed.
   */
  CONNECTION_EVENT,

  /**
   * Placeholder for unknown-to-sidecar message types for messages intended to be for extension ->
   * extension messaging via sidecar.
   */
  UNKNOWN;
