...
public class KafkaConsumerFactory {
  ...
  public KafkaConsumer<byte[], byte[]> getClient(
    ...
    var config = configurator.getConsumerClientConfig(connectionId, clusterId, false);
    // And apply the overrides
    configOverrides.forEach((key, value) -> config.put(key.toString(), value.toString()));

    Log.debugf("Creating consumer for connection %s and cluster %s with configuration:\n  %s",
        connectionId, clusterId, config
    );
