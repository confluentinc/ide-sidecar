...
public class RecordsV3ApiImpl {
  ...
  public Uni<ProduceResponse> produceRecord(
      ...
      @PathParam("cluster_id") String clusterId, @PathParam("topic_name") String topicName,
      @Valid ProduceRequest produceRequest
  ) {
    return uniItem(
        ProduceContext.fromRequest(connectionId, clusterId, topicName, produceRequest)).chain(
        this::ensureTopicPartitionExists).chain(this::ensureKeyOrValueDataExists).chain(
        this::fetchClients).chain(this::getSchemas).chain(this::serialize).chain(
        this::sendSerializedRecord).map(this::toProduceResponse);
  }
  ...
  private Uni<ProduceContext> ensureTopicPartitionExists(ProduceContext c) {
    return topicManager
        ...
        .chain(ignored -> Optional.ofNullable(c.produceRequest.getPartitionId()).map(
            partitionId -> partitionManager.getKafkaPartition(c.clusterId, c.topicName,
                partitionId
            )).orElse(Uni.createFrom().nullItem())).onItem().transform(ignored -> c);
  }
  ...
  private Uni<ProduceContext> fetchClients(ProduceContext c) {
    return combineUnis(
     ...
     .runSubscriptionOn(Infrastructure.getDefaultWorkerPool()).map(tuple -> c.with().srClient(
            tuple.getItem1()).producer(tuple.getItem2()).build());
  }
  ...
  private Uni<ProduceContext> getSchemas(ProduceContext c) {
    return combineUnis(
        () -> schemaManager.getSchema(c.srClient, c.topicName, c.produceRequest.getKey(), true),
        () -> schemaManager.getSchema(c.srClient, c.topicName, c.produceRequest.getValue(), false)
    ).asTuple()
     ...
     .runSubscriptionOn(Infrastructure.getDefaultWorkerPool()).onFailure(RuntimeException.class)
     .transform(Throwable::getCause).map(tuple -> c.with().keySchema(tuple.getItem1())
                                                   .valueSchema(tuple.getItem2()).build());
  }
  ...
  private Uni<ProduceContext> serialize(ProduceContext c) {
    return combineUnis(() -> recordSerializer.serialize(c.srClient,
        c.keySchema.map(SchemaManager.RegisteredSchema::parsedSchema).orElse(null), c.topicName,
        c.produceRequest.getKey().getData(), true
    ), () -> recordSerializer.serialize(c.srClient,
        c.valueSchema.map(SchemaManager.RegisteredSchema::parsedSchema).orElse(null), c.topicName,
        c.produceRequest.getValue().getData(), false
    )).with((key, value) -> c.with().serializedKey(key).serializedValue(value).build());
  }

  private Uni<ProduceContext> sendSerializedRecord(ProduceContext c) {
    return uniStage(sendSerializedRecord(c.producer, c.topicName, c.produceRequest.getPartitionId(),
        c.produceRequest.getTimestamp(), Optional.ofNullable(c.serializedKey())
                                                 .map(ByteString::toByteArray).orElse(null),
        Optional.ofNullable(c.serializedValue()).map(ByteString::toByteArray).orElse(null)
    )).map(recordMetadata -> c.with().recordMetadata(recordMetadata).build());
  }

  private static CompletableFuture<RecordMetadata> sendSerializedRecord(
      KafkaProducer<byte[], byte[]> producer, String topicName, Integer partitionId, Date timestamp,
      byte[] key, byte[] value
  ) {
    var completableFuture = new CompletableFuture<RecordMetadata>();
    producer.send(new ProducerRecord<>(topicName, partitionId, Optional.ofNullable(timestamp)
                                                                       .orElse(
  ...
  private ProduceResponse toProduceResponse(ProduceContext c) {
    return ProduceResponse.builder().clusterId(c.clusterId).topicName(c.topicName).partitionId(
        c.recordMetadata.partition()).offset(c.recordMetadata.offset()).timestamp(
        new Date(c.recordMetadata.timestamp())).key(
        toProduceResponseData(c.keySchema(), c.recordMetadata, true)).value(
        toProduceResponseData(c.valueSchema(), c.recordMetadata, false)).build();
  }

  private static ProduceResponseData toProduceResponseData(
      Optional<SchemaManager.RegisteredSchema> schema, RecordMetadata metadata, boolean isKey
  ) {
    return ProduceResponseData.builder().size(
        (long) (isKey ? metadata.serializedKeySize() : metadata.serializedValueSize())).schemaId(
        schema.map(SchemaManager.RegisteredSchema::schemaId).orElse(null)).subject(
        schema.map(SchemaManager.RegisteredSchema::subject).orElse(null)).schemaVersion(
        schema.map(SchemaManager.RegisteredSchema::schemaVersion).orElse(null)).type(
        schema.map(s -> s.parsedSchema().schemaType()).orElse(null)).build();
  }
  ...
  record ProduceContext(
      // Provided
      String connectionId, String clusterId, String topicName, ProduceRequest produceRequest,
      // Fetched
      KafkaProducer<byte[], byte[]> producer, SchemaRegistryClient srClient,
      // Computed fields
      Optional<SchemaManager.RegisteredSchema> keySchema,
      Optional<SchemaManager.RegisteredSchema> valueSchema, ByteString serializedKey,
      ByteString serializedValue, RecordMetadata recordMetadata) implements
                                                                 RecordsV3ApiImplProduceContextBuilder.With {

    static ProduceContext fromRequest(
        String connectionId, String clusterId, String topicName, ProduceRequest produceRequest
    ) {
      return RecordsV3ApiImplProduceContextBuilder.ProduceContext(connectionId, clusterId,
          topicName, produceRequest, null, null, Optional.empty(), Optional.empty(), null, null,
          null
      );
    }
  }
