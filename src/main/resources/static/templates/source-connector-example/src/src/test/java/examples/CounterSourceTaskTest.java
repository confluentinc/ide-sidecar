...
import static examples.CounterSourceTask.CURRENT_ITERATION;
...
public class CounterSourceTaskTest {
  ...
  public void testTask(Long startingOffset) throws InterruptedException {
    ...
    if (startingOffset == null) {
      ...
      Map<String, Object> initialOffset = new HashMap<>();
      initialOffset.put(CURRENT_ITERATION, startingOffset.longValue());
      when(mockReader.offset(anyMap())).thenReturn(initialOffset);
    }
    when(mockContext.offsetStorageReader()).thenReturn(mockReader);

    Map<String, String> properties = new HashMap();
    properties.put("kafka.topic", "my-topic");
    properties.put("interval.ms", "1000");
