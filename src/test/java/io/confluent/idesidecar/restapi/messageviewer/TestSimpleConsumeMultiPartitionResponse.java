...
public class TestSimpleConsumeMultiPartitionResponse {
  ...
  void testNoSchemaDataToMultiPartitionConsumeData() throws IOException {
    ...
    // Deserialize JSON 1
    SimpleConsumeMultiPartitionResponse data1 = mapper.readValue(json1,
    ...
    SimpleConsumeMultiPartitionResponse.PartitionConsumeData partitionData1 = data1.partitionDataList()
        .get(0);
    assertEquals(0, partitionData1.partitionId());
    assertEquals(60607, partitionData1.nextOffset());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecord record1 = partitionData1.records()
        .get(0);
    ...
    assertEquals(
        "{\"ordertime\":1497402000262,\"orderid\":363302,\"itemid\":\"Item_7\",\"orderunits\":0.7202503545348564,\"address\":{\"city\":\"City_\",\"state\":\"State_35\",\"zipcode\":94086}}",
        record1.value().asText());

    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecordHeader header1_1 = record1.headers()
        .get(0);
    assertEquals("task.generation", header1_1.key());
    assertEquals("MA==", header1_1.value());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecordHeader header1_2 = record1.headers()
        .get(1);
    assertEquals("task.id", header1_2.key());
    assertEquals("MA==", header1_2.value());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecordHeader header1_3 = record1.headers()
        .get(2);
  ...
  void testSchemaEncodedDataToMultiPartitionConsumeData() throws IOException {
    ...
    SimpleConsumeMultiPartitionResponse data2 = mapper.readValue(json2,
        SimpleConsumeMultiPartitionResponse.class);
    assertEquals("lkc-abcd123", data2.clusterId());
    assertEquals("order_json_sr", data2.topicName());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeData partitionData2 = data2.partitionDataList()
        .get(0);
    assertEquals(0, partitionData2.partitionId());
    assertEquals(60652, partitionData2.nextOffset());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecord record2 = partitionData2.records()
        .get(0);
    ...
    assertEquals(
        "AAABhqF7Im9yZGVydGltZSI6MTQ4ODA1MjkzMjEyMywib3JkZXJpZCI6MzYzMjg3LCJpdGVtaWQiOiJJdGVtXzU0MyIsIm9yZGVydW5pdHMiOjQuNzMwMDI5ODkxOTUwODE4LCJhZGRyZXNzIjp7ImNpdHkiOiJDaXR5XyIsInN0YXRlIjoiU3RhdGVfMTgiLCJ6aXBjb2RlIjo3NTA2MH19",
        value2.get("__raw__").asText());

    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecordHeader header2_1 = record2.headers()
        .get(0);
    assertEquals("task.generation", header2_1.key());
    assertEquals("MA==", header2_1.value());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecordHeader header2_2 = record2.headers()
        .get(1);
    assertEquals("task.id", header2_2.key());
    assertEquals("MA==", header2_2.value());
    SimpleConsumeMultiPartitionResponse.PartitionConsumeRecordHeader header2_3 = record2.headers()
        .get(2);
