...
public class ConfluentLocalKafkaWithRestProxyContainer
    ...
    static ServerSocket kafka_rest_proxy_port_socket;

    static {
        try {
            kafka_rest_proxy_port_socket = new ServerSocket(0);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    static ServerSocket kafka_port_socket;

    static {
        try {
            kafka_port_socket = new ServerSocket(0);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static final int KAFKA_PORT = kafka_port_socket.getLocalPort();
    public static final int REST_PROXY_PORT = kafka_rest_proxy_port_socket.getLocalPort();


    public ConfluentLocalKafkaWithRestProxyContainer() throws IOException {
        this(DEFAULT_IMAGE);
    }

    public ConfluentLocalKafkaWithRestProxyContainer(String dockerImageName) throws IOException {
